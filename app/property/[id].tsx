...
export default function PropertyDetail() {
  ...
  const ownProperty =
    !!session?.user?.id &&
    property?.owner_id === session.user.id
  ...
  async function toggleFavorite() {
    if (!session?.user?.id || !property) {
      Alert.alert(
        'Belépés szükséges',
        'A mentéshez előbb jelentkezz be.'
      )
  ...
  async function removeProperty() {
    ...
    const remove = async () => {
      try {
        await deletePropertyWithImages(
          property.id
        )
      } catch {
        return Alert.alert(
          'Hiba',
          'A hirdetést és a képeit nem sikerült törölni.'
        )
      }
  ...
  function shareContent() {
    const url =
      `https://www.realvia.hu/property/${property?.id || id}`
    ...
    const text =
      `Találtam egy ingatlant a Realvián, amely szerintem érdekes lehet számodra.\n\n🏡 ${title}\n📍 ${location}\n💰 ${price}\n\nAz ingatlan részletei és fotói itt tekinthetők meg:\n${url}`
  ...
  async function copyShareLink() {
    ...
    try {
      ...
      setShareNotice(
        'A linket a vágólapra másoltuk.'
      )
    } catch {
      setShareNotice(
        `Másold ki ezt a linket: ${url}`
      )
    }
  }
  ...
  async function shareByEmail() {
    ...
    await Linking.openURL(
      `mailto:?subject=${encodeURIComponent(
        `Ingatlanajánlat a Realviától – ${location}`
            ...
            >
              ...
              >
                {ownProperty
                  ? 'Saját hirdetésed'
                  : sold
                    ? 'Sikeresen értékesítve'
                    : 'Egyeztess megtekintést'}
              </Text>
              ...
              >
                {ownProperty
                  ? 'Itt szerkesztheted vagy törölheted a hirdetést. A beérkezett megkereséseket az Érdeklődések oldalon kezelheted.'
                  : sold
                    ? 'Ez a hirdetés referenciaértékkel továbbra is megtekinthető. Hasonló ingatlan iránt továbbra is jelezheted az érdeklődésed.'
                    : 'Kérj visszahívást, további információt vagy adj meg számodra megfelelő megtekintési időpontokat.'}
              </Text>
              ...
              {!ownProperty && (
                ...
                >
                  ...
                  >
                    {sold
                      ? 'Hasonló ingatlant keresek'
                      : 'Megtekintés vagy érdeklődés'}
                  </Text>
                </Pressable>
              )}
      ...
      >
        ...
        >
          ...
          >
            ...
            >
              Küldd tovább egyetlen
              érintéssel
            </Text>

            <Text
              style={
                styles.shareProperty
              }
              numberOfLines={2}
            >
              {property.title}
